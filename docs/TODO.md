...
- [x] 「今の気分」表示が区平均か自分の投稿か分かりにくい問題を解消 (2026-08-08)
  - `src/client/MapView.tsx`: ヘッダーの投稿ブロックに「今の気分(あなたの最新の投稿)」タグを追加し、下の「東京全体の気分」(区平均)カードと区別
  - `src/client/MyPage.tsx`: プロフィールタブの直近の記録セクションに同様のタイトルを追加(他セクションと統一感のある見出し)
  - Playwrightでマップ・マイページ両方のラベル表示を確認済み（コンソールエラーなし）
  - GitHubにpush済み (`1c5767c`)、本番デプロイ済み (Version ID: 74f828f3-3e36-4ab1-8a61-082ce4c84190)
